...
class GUIAbstract(ABC):
    ...
    def StockQuote(self):
        ...
        Quote(server, 1, 0, 'ANZ')

    def StockGraph(self):
        server = Server(url).getServer()
        Graph(server, 1, 0, 'CBA')
...
def Graph(server, server_choice, checkbox, quote):
    ax.clear()
    window = Toplevel(root)
    monitor = StockGraphMonitor(ax, f, window, root, server, server_choice, checkbox, quote)
    print("hey")
    flag = False
    while not flag:
        monitor.createGraph()
...
if __name__ == "__main__":
    root = Tk()
    root.title("Stock Quote Monitor")
    # root.resizable(0,0)
    root.geometry("%dx%d%+d%+d" % (600, 400, 200, 100))
    g = GUI(root)
    root.mainloop()
